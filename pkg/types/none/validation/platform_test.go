...
func TestValidateProvisioning(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			expected: "none.fencingCredentials\\[1\\].BMC.Address: Duplicate value: \"ipmi://192.168.111.1\"",
		},
		{
			name: "bmc_address_required",
			platform: platform().
				FencingCredentials(fc1().BMCAddress("")).build(),
			expected: "none.fencingCredentials\\[0\\].BMC.Address: Required value: missing Address",
		},
		{
			name: "bmc_username_required",
			platform: platform().
				FencingCredentials(fc1().BMCUsername("")).build(),
			expected: "none.fencingCredentials\\[0\\].BMC.Username: Required value: missing Username",
		},
		{
			name: "bmc_password_required",
			platform: platform().
				FencingCredentials(fc1().BMCPassword("")).build(),
			expected: "none.fencingCredentials\\[0\\].BMC.Password: Required value: missing Password",
		},
		{
